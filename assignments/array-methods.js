...
// Problem 1 Alphabetize first names
let alphaRunners = runners.map(function(runner) {
    return runner.first_name;    
});

alphaRunners.sort();

console.log(alphaRunners);

// Problem 2 Display first name, last name, and company
// let companyName = runners.forEach (function(runner) {
//     return `${runner.first_name}` `${runner.last_name}` `${runner.company_name}`;
// });

let companyInfo = [];
runners.forEach(function(runner) {
    companyInfo.push(`${runner.first_name} ${runner.last_name} ${runner.company_name}`);
});

console.log(companyInfo);


// Problem 3 People who donated $200 or more

let largeDonation = runners.filter(function(donationSize) {
    return donationSize.donation >= 200;
});
